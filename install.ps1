...
if ((Test-Path '.env') -and (Select-String -Path '.env' -Pattern '^NEUSLICE_TOKEN=' -Quiet)) {
    ...
    if (-not (Select-String -Path '.env' -Pattern '^WATCHTOWER_TOKEN=' -Quiet)) {
        $wt = -join ((1..64) | ForEach-Object { '{0:x}' -f (Get-Random -Maximum 16) })
        Add-Content -Path '.env' -Value "`nWATCHTOWER_TOKEN=$wt"
...
if (-not $skipEnv) {
    ...
    # ── Write .env ────────────────────────────────────────────────────────────
    $watchtowerToken = -join ((1..64) | ForEach-Object { '{0:x}' -f (Get-Random -Maximum 16) })
    $envLines = @(
